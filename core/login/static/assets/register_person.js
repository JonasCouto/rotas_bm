document.addEventListener("DOMContentLoaded", function () {
      ...
      fetch(`https://viacep.com.br/ws/${cep}/json/`) // Consultar a API de CEP
          ...
          .then((data) => {
              if (!("erro" in data)) {
                  ...
                  if (numeroInput.value && residenciaInput.value) {
                      const address = `${residenciaInput.value}, ${numeroInput.value}, ${bairroInput.value}, ${cidadeInput.value}, ${estadoInput.value}`;
                      geocodeAddress(address); // Geocodificação do endereço completo
                  } else {
                      const address = `${cidadeInput.value}, ${estadoInput.value}`;
                      geocodeAddress(address); // Geocodificação da cidade e estado
                  }
              } else {
  ...
  formInputs.forEach((input) => {
      input.addEventListener("change", function () {
          if (numeroInput.value && residenciaInput.value && cidadeInput.value && estadoInput.value) {
              const address = `${residenciaInput.value}, ${numeroInput.value}, ${bairroInput.value}, ${cidadeInput.value}, ${estadoInput.value}`;
              geocodeAddress(address); // Geocodificação do endereço completo
          } else if (cidadeInput.value && estadoInput.value) {
              const address = `${cidadeInput.value}, ${estadoInput.value}`;
              geocodeAddress(address); // Geocodificação da cidade e estado
          }
      });
